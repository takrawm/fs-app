...
export class SeedDataLoader {
  ...
  public getHistoricalPeriods(): Period[] {
    return this.periods.filter(period => period.isHistorical);
  }
  ...
  public getPeriodByYearMonth(year: number, month: number): Period | undefined {
    return this.periods.find(period => period.year === year && period.month === month);
  }

  // パラメータ取得メソッド
  public getParametersForAccount(accountId: string): Record<string, Parameter> | undefined {
    return this.parameterMap.get(accountId);
  }

  public getParameter(accountId: string, parameterKey: string): Parameter | undefined {
  ...
  public validateData(): ValidationResult {
    ...
    this.accounts.forEach(account => {
      ...
      if (account.parentId && !this.getAccountById(account.parentId)) {
        errors.push(`Account ${account.id} has invalid parentId: ${account.parentId}`);
      }
      ...
      if (account.parameter.type === 'formula') {
        account.parameter.dependencies.forEach(depId => {
          if (!this.getAccountById(depId)) {
            warnings.push(`Account ${account.id} has dependency on non-existent account: ${depId}`);
          }
        });
      }
      ...
      if (account.cfImpact.targetAccountIds) {
        account.cfImpact.targetAccountIds.forEach(targetId => {
          if (!this.getAccountById(targetId)) {
            warnings.push(`Account ${account.id} has CF impact on non-existent account: ${targetId}`);
          }
        });
      }
    });
...
export const getSeedAccountById = (id: string) => seedDataLoader.getAccountById(id);
export const getSeedPeriodById = (id: string) => seedDataLoader.getPeriodById(id);