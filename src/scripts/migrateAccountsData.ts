...
import type { Account, SheetType, DisplayOrder, CfImpact } from '../types/account';
...
class AccountDataMigrator {
  ...
  private newAccountsPath = path.join(process.cwd(), 'src/seed/accounts.new.json');
  private backupPath = path.join(process.cwd(), 'src/seed/accounts.backup.json');
  ...
  private migrateParameter(oldAccount: any, paramMap: Map<string, any>): Parameter {
    // アカウント内のパラメータを優先
    ...
    switch (param.type) {
      ...
      case 'formula':
        return {
          ...
          dependencies: this.extractDependencies(param.formula || '', param.dependencies)
        } as FormulaParameter;
  ...
  private extractDependencies(formula: string, existingDeps?: string[]): string[] {
  ...
  private addMissingAccounts(): void {
    ...
    missingIds.forEach(id => {
      ...
      if (id === 'expense-depreciation') {
        ...
        this.log('不足アカウント追加', { id, name: depreciationAccount.accountName });
  ...
  private async validateNewData(): Promise<boolean> {
    ...
    if (!result.isValid) {
      ...
      this.errors.push(...result.errors.map(e => `${e.context}: ${e.field} - ${e.message}`));
    }

    if (result.warnings.length > 0) {
      console.warn('⚠️  警告:');
      result.warnings.forEach(warning => {
        console.warn(`  - ${warning.context}: ${warning.field} - ${warning.message}`);
      });
      this.warnings.push(...result.warnings.map(w => `${w.context}: ${w.field} - ${w.message}`));
    }
  ...
  private async saveNewData(): Promise<void> {
    ...
    this.log('データ保存', { path: this.newAccountsPath, count: this.newAccounts.length });
  }
...
async function main() {
  try {
    ...
    if (result.success) {
      ...
      console.log('2. 問題がなければ accounts.json を accounts.new.json で置き換え');
