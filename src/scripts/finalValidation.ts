...
class FinalValidator {
  private paths = {
    ...
    parameterOverrides: path.join(process.cwd(), 'src/seed/parameterOverrides.json'),
  ...
  private async loadAllData(): Promise<void> {
    ...
    try {
      ...
      if (fs.existsSync(this.paths.parameterOverrides)) {
        ...
        console.log(`✓ parameterOverrides: ${this.parameterOverrides.length}件`);
      }
  ...
  private checkDataIntegrity(): void {
    ...
    if (result.isValid) {
      ...
      this.addCheckResult({
        ...
        errors: result.errors.map(e => `${e.context}: ${e.field} - ${e.message}`)
      });
    }
  }
  ...
  private async performanceCheck(): Promise<void> {
    ...
    const metrics: PerformanceMetrics = {
      ...
      totalRecords: this.newAccounts.length + this.periods.length + this.financialValues.length
    };
  ...
  private checkCalculationAccuracy(): void {
    ...
    this.addCheckResult({
      ...
      details: warnings.length === 0 ? '計算結果は妥当です' : `${warnings.length}件の警告`,
      errors: warnings
    });
  }
  ...
  private checkMigrationChecklist(): void {
    ...
    const checklist = [
      { item: '新accounts.jsonが生成されている', check: fs.existsSync(this.paths.newAccounts) },
      { item: 'financialValues.jsonが生成されている', check: fs.existsSync(this.paths.financialValues) },
      { item: 'parameters.jsonの情報が統合されている', check: this.checkParametersIntegration() },
  ...
  private checkTypeCompliance(): boolean {
    // 簡易的な型チェック
    return this.newAccounts.every(account => 
      account.id && 
      account.accountName && 
      account.sheet && 
      account.parameter && 
  ...
  private generateReport(): ValidationReport {
    ...
    const warningChecks = this.checkResults.filter(r => r.status === 'WARNING');
    ...
    if (canProceedWithDeletion) {
      recommendations.push('すべてのチェックをパスしました。parameters.jsonの削除が可能です');
      recommendations.push('削除前に最終バックアップを作成することを推奨します');
    }
  ...
  private async promptForDeletion(): Promise<void> {
    ...
    console.log('\n# バックアップの作成');
    console.log('cp src/seed/parameters.json src/seed/backup/parameters.$(date +%Y%m%d_%H%M%S).json');
    console.log('cp src/seed/accounts.json src/seed/backup/accounts.$(date +%Y%m%d_%H%M%S).json');
    console.log('\n# ファイルの更新');
  ...
  private generateMarkdownReport(report: ValidationReport): string {
    ...
    lines.push(`## 総合結果: ${report.overallStatus === 'PASS' ? '✅ PASS' : '❌ FAIL'}\n`);
