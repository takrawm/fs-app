...
class FinancialValueGenerator {
  ...
  private async generateValues(
    sortedAccounts: AccountWithDependencies[]
  ): Promise<void> {
    console.log("財務値の生成中...");
  ...
  private async saveData(): Promise<void> {
    ...
    console.log(
      `  - ファイルサイズ: ${(jsonData.length / 1024).toFixed(2)} KB\n`
    );
  }
  ...
  generateReport(): string {
    ...
    lines.push(
      `- 理論上の最大レコード数: ${this.accounts.length * this.periods.length}`
    );
