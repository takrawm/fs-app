...
class ParameterOverrideManager {
  ...
  private generateReport(): void {
    ...
    lines.push(
      `- 影響を受けるアカウント数: ${
        new Set(this.overrides.map((o) => o.accountId)).size
      }件`
    );
    lines.push(
      `- 影響を受ける期間数: ${
        new Set(this.overrides.map((o) => o.periodId)).size
      }件\n`
    );

    lines.push("## シナリオ別統計\n");
    this.scenarios.forEach((scenario) => {
      const count = this.overrides.filter((o) =>
        o.reason.includes(scenario.reason.slice(0, 20))
      ).length;
    ...
    this.overrides.slice(0, 10).forEach((override, index) => {
      lines.push(
        `${index + 1}. **${override.accountId}** (${override.periodId})`
      );
...
export class OverrideApplicator {
  ...
  getOverridesForPeriod(periodId: string): ParameterOverride[] {
    return Array.from(this.overrides.values()).filter(
      (override) => override.periodId === periodId
    );
  }
